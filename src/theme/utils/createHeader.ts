...
const createHeader = (cv: CV, colours: Colours): RevivableComponent[] => [
  {
    ...
    children: [
      ...
      createComment(colours.highlight, [
        { type: "text", props: { color: colours.highlight, text: "@var", size: 12, } },
        { type: "block", props: { width: 6 } },
        { type: "text", props: { color: colours.highlight, text: `{${cv.role}} ${cv.nick}`, size: 12, } },
      ]),
      { type: "block", props: { height: 6 } },
      { type: "text", props: { color: colours.text, text: `${cv.name}`, size: 24 } },
    ],
  },
];
